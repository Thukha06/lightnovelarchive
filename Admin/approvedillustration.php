...
session_start();
  ...
  $sql = "SELECT * FROM image_table WHERE accept_reject='$one'";
  $stmt = $db->query($sql);
  $result = $stmt->setFetchMode(PDO::FETCH_ASSOC);

  if (($_SERVER["REQUEST_METHOD"] == "GET") && (!empty($_GET['dropillustration']))) {
    $imageid=$_GET['dropillustration'];
    $sql2 = "DELETE FROM image_table WHERE image_id='$imageid'";
    $db->exec($sql2);
    echo "<script>location.href='approvedillustration.php';</script>";
  }
  if (($_SERVER["REQUEST_METHOD"] == "GET") && (!empty($_GET['unpublishillustration']))) {
    $imageid=$_GET['unpublishillustration'];
    $sql3 = "UPDATE image_table SET accept_reject='$zero'WHERE image_id='$imageid'";
    $db->exec($sql3);
    echo "<script>location.href='approvedillustration.php';</script>";
  }
...
<head>
 ...
 <script type="text/javascript">
  function ConfirmAccept(){
    var msg=confirm("Are you sure want to unpublish?");
    if(msg)
  return true;
    else return false;
  }
        ...
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item ">
            ...
            <ul class="nav nav-treeview">
              ...
              <li class="nav-item">
                <a href="approvedillustration.php" class="nav-link active">
                  <i class="far fa-circle nav-icon"></i>
          ...
          <div class="col-12">
            ...
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">DataTable for Approved Illustration</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tr>
                    ...
                    <td><?php echo "<a href='previewimageintable.php?imgpath=$filename'>$imgpath</a>" ?></td>
                    <td><?php echo $row['img_date']; ?></td>
                    <td>                      
                        <div class="row">
                        <div class="col-7">
                          <?php echo "<a class='button' href='approvedillustration.php?unpublishillustration=$imageid' onclick='return ConfirmAccept();'><button class='btn btn-block btn-success btn-sm'>Unpublish</button></a>" ?>
                        </div>
                        <div class="col-5">
                          <?php echo "<a class='button' href='approvedillustration.php?dropillustration=$imageid' onclick='return ConfirmReject();'><button class='btn btn-block btn-danger btn-sm'>Drop</button></a>" ?>
                        </div>
                        </div>
                    </td>
                  </tr>
