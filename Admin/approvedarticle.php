<?php
  ...
  $sql1 = "SELECT * FROM article_table";
  $stmt1 = $db->query($sql1);
  $result1 = $stmt1->setFetchMode(PDO::FETCH_ASSOC);

  if (($_SERVER["REQUEST_METHOD"] == "GET") && (!empty($_GET['accept_articleId']))) {
    $articleId = $_GET['accept_articleId'];

    $sql3 = "UPDATE article_table SET publish_unpublish=0 WHERE article_id='$articleId'";
    $db->exec($sql3);
    echo "<script>location.href='approvedarticle.php';</script>";
  }

  if (($_SERVER["REQUEST_METHOD"] == "GET") && (!empty($_GET['reject_articleId'])) && (!empty($_GET['chapId']))) {
    $articleId = $_GET['reject_articleId']; $ch_Id = $_GET['chapId'];

    $sql5 = "DELETE FROM chapter_table WHERE ch_id='$ch_Id'";
    $db->exec($sql5);

    $sql4 = "DELETE FROM article_table WHERE article_id='$articleId'";
    $db->exec($sql4);
    echo "<script>location.href='approvedarticle.php';</script>";
  }
?>
...
<head>
  ...
  <script type="text/javascript">
    function ConfirmAccept(){
      var msg=confirm("Are you sure you want to Unpublish?");
      if(msg)
    return true;
      else return false;
    }
    ...
    function ConfirmReject(){
      var msg=confirm("Are you sure you want to Drop the Article?\nThe action cannot be reversible.");
      if(msg)
    return true;
      else return false;
    }
        ...
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item menu-open">
            ...
            <ul class="nav nav-treeview">
              ...
              <li class="nav-item">
                <a href="approvedarticle.php" class="nav-link active">
                  <i class="far fa-circle nav-icon"></i>
          ...
          <div class="col-12">
            ...
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">DataTable for Approved Article</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <?php while($row1 = $stmt1->fetch()) { 
                        $status = $row1['publish_unpublish']; 
                        if ($status==1) { ?>
                    <tr>
                      <?php $articleId = $row1['article_id'];
                            $articleTitle = $row1['article_title']; ?>
                      <td><?php echo "<a href='../showarticle.php?articleId=$articleId#target1'>$articleTitle</a>"; ?></td>
                      <?php 
                      $sql2 = "SELECT * FROM chapter_table WHERE article_id='$articleId'";
                      $stmt2 = $db->query($sql2);
                      $result2 = $stmt2->setFetchMode(PDO::FETCH_ASSOC);
                      $row2 = $stmt2->fetch();
                      $chapId = $row2['ch_id'];
                      $chapTitle = $row2['ch_title'];
                       ?>
                      <td><?php echo "<a href='../readchapter.php?chapId=$chapId'>$chapTitle</a>"; ?></td>
                      <td><?php echo $row1['user_name']; ?></td>
                      <td><?php echo $row1['published_date']; ?></td>
                      <td>
                        <div class="row">
                        <div class="col-7">
                          <?php echo "<a class='button' href='approvedarticle.php?accept_articleId=$articleId' onclick='return ConfirmAccept();'><button class='btn btn-block btn-primary btn-sm'>Unpublish</button></a>" ?>
                        </div>
                        <div class="col-5">
                          <?php echo "<a class='button' href='approvedarticle.php?reject_articleId=$articleId&chapId=$chapId' onclick='return ConfirmReject();'><button class='btn btn-block btn-danger btn-sm'>Drop</button></a>" ?>
                        </div>
                      </div>
                      </td>
                    </tr>
