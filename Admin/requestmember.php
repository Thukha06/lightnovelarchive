<?php
  ...
  if (($_SERVER["REQUEST_METHOD"] == "GET") && (!empty($_GET['rejectrequest']))) {
    $requestid=$_GET['rejectrequest'];
    $sql2 = "DELETE FROM role_request WHERE role_request_id='$requestid'";
    $db->exec($sql2);
    echo "<script>location.href='requestmember.php';</script>";
  }

  if (($_SERVER["REQUEST_METHOD"] == "GET") && (!empty($_GET['acceptrequest'])) && (!empty($_GET['requestedrole']))) {
    $requestid=$_GET['acceptrequest']; $requestedrole=$_GET['requestedrole'];
    $username=$_GET['username']; $useremail=$_GET['useremail']; $currentrole=$_GET['currentrole'];

    $sql3 = "UPDATE login_table SET user_type='$requestedrole' WHERE (user_name='$username' or user_email='$useremail')";
    $db->exec($sql3);

    $sql4 = "INSERT INTO role_approved (aprov_user_name, aprov_user_email, aprov_current_role, aprov_prev_role) VALUES ('$username', '$useremail', '$requestedrole', '$currentrole')";
    $db->exec($sql4);
        ...
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item menu-open">
            ...
            <ul class="nav nav-treeview">
              ...
              <li class="nav-item">
                <a href="requestmember.php" class="nav-link active">
                  <i class="far fa-circle nav-icon"></i>
                  <p>Member Requests</p>
                </a>
              </li>
          ...
          <div class="col-12">
            ...
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">DataTable for Requested Member</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <?php $requestid = $row['role_request_id'];
                            $username = $row['r_user_name']; ?>
                      <td><?php echo $username ?></td>
                      <?php $useremail = $row['r_user_email']; ?>
                      <td><?php echo $useremail ?></td>
                      <?php 
                        $currentrole = $row['r_current_role'];
                        foreach ($users as $row1) {
                        if ($currentrole == ($row1['user_type'])) {
                      ?>
                      <td><?php echo $row1['user_type_name']; } } ?></td>
                      <?php
                        foreach ($users as $row2) {
                        if (($row['r_requested_role']) == ($row2['user_type'])) {
                          $requestedrole = $row2['user_type'];
                      ?>
                      <td><?php echo $row2['user_type_name']; } } ?></td>
                      <td>
                      
                      <div class="row">
                        <div class="col-6">
                          <?php echo "<a class='button' href='requestmember.php?username=$username&useremail=$useremail&acceptrequest=$requestid&currentrole=$currentrole&requestedrole=$requestedrole' onclick='return ConfirmAccept();'><button class='btn btn-block btn-success btn-sm'>Accept</button></a>" ?>
                        </div>
                        <div class="col-6">
                          <?php echo "<a class='button' href='requestmember.php?rejectrequest=$requestid' onclick='return ConfirmReject();'><button class='btn btn-block btn-danger btn-sm'>Reject</button></a>" ?>
                        </div>
                      </div>
                      </td>
                    </tr>
