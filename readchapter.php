<?php
    ...
    if (($_SERVER["REQUEST_METHOD"] == "GET") && (!empty($_GET['chapId']))) {
        $chapId = $_GET['chapId'];

        $sql = "SELECT * FROM chapter_table WHERE ch_id='$chapId'";
        $stmt = $db->query($sql);
        $result = $stmt->setFetchMode(PDO::FETCH_ASSOC);
        $row = $stmt->fetch();

        $content = $row['ch_content'];
        $reviewImg = $row['ch_photo'];
        $paratitle = $row['ch_title'];
        $imgpos = $row['photo_pos'];
        $orien = $row['photo_orien'];
        $articleId = $row['article_id'];

        $sql1 = "SELECT * FROM article_table WHERE article_id='$articleId'";
        $stmt1 = $db->query($sql1);
        $result1 = $stmt1->setFetchMode(PDO::FETCH_ASSOC);
        $row1 = $stmt1->fetch();
        $title = $row1['article_title'];
...
<section>
  <div class="title">
    <center>
    <h1 style="width: 38%;"><?php echo $title; ?></h1></center>
